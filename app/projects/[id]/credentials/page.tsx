...
export default function ProjectCredentialsPage({
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [fetchedProject, fetchedCredentials] = await Promise.all([
      ...
      toastError("Failed to fetch project credentials");
    } finally {
      setLoading(false);
    }
  }, [projectId, search, typeFilter, toastError]);
  ...
  const handleFormSubmit = async (formData: CredentialFormData) => {
    ...
    if (formMode === "add") {
      ...
      if (res.success) {
        success("Credential Created", `${formData.name} stored in project vault.`);
        loadData();
      } else {
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span>Back to {project?.project_name || "Project"} Overview</span>
        </Link>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-xl border border-[#E6EAF2] bg-white">
        <div className="space-y-1">
          ...
          <h1 className="text-xl sm:text-2xl font-semibold text-[#0F172A] tracking-tight">
            {project?.project_name || "Project"} Credentials
          </h1>
      ...
      {/* 2. Filter Bar */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-4 rounded-xl border border-[#E6EAF2] bg-white text-xs">
        <div className="flex-1 min-w-[200px]">
          <Input
            placeholder="Search credential name, url, username..."
            value={search}
      ...
      {loading ? (
        <div className="rounded-xl border border-[#E6EAF2] bg-white p-12 text-center text-xs text-slate-400">
          ...
          <span>Accessing project vault...</span>
        </div>
        ...
        <div className="rounded-xl border border-[#E6EAF2] bg-white p-12 text-center">
          ...
          <h3 className="text-sm font-bold text-slate-900 font-display">
            No Credentials Stored for this Project
          </h3>
          <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
            {search || typeFilter !== "All"
              ? "No credentials match your active filter."
              : "Store database credentials, staging login, server SSH, or API keys for this project."}
          </p>
          {canManage && !search && typeFilter === "All" && (
            <Button
              variant="default"
              size="sm"
              onClick={handleOpenAddModal}
